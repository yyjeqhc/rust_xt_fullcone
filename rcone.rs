...
#[derive(Debug)]
struct RConeNatTarget {
    // mappings: KVec<NatMapping>,  // NAT映射表
    target:u64,            // target结构体
}

impl kernel::Module for RConeNatTarget {
    fn init(module: &'static ThisModule) -> Result<Self> {
        // pr_info!("RConeNat: module being initialized\n");
        ...
        // 设置回调函数
        target.target = Some(rconenat_tg);
        target.checkentry = Some(rconenat_tg_check); 
        target.destroy = Some(rconenat_tg_destroy);
        ...
        pr_info!("RConeNat: module initialized success\n");
        
        let full = RConeNatTarget {
            // mappings: KVec::new(),
...
impl Drop for RConeNatTarget {
    fn drop(&mut self) {
        pr_info!("RConeNat: module being removed\n");
...
// target处理函数
extern "C" fn rconenat_tg(skb: *mut sk_buff, par: *const xt_action_param) -> u32 {
...
// 规则检查函数
extern "C" fn rconenat_tg_check(par: *const xt_tgchk_param) -> i32 {
    // pr_info!("RConeNat: checking new rule");
    unsafe {
        let target = (*par).target; // 获取 xt_target 指针

        // if target.is_null() {
        //     pr_err!("RConeNat: target is null!\n");
        //     return -1;
        // }
        // pr_info!("check {:X}\n",target as u64);
        // pr_info!("{:X}\n",(*target).me as u64);
        // 使用 container_of 获取 `RConeNatTarget` 实例

        // let module_ptr: *mut RConeNatTarget = (*target).me as *mut RConeNatTarget;
        ...
        //     interface: 0,
        // },flags::GFP_KERNEL);
        // pr_info!("RConeNat: Successfully accessed RConeNatTarget {:X} num {}\n",module.target,module.mappings.len());
...
// 销毁函数
extern "C" fn rconenat_tg_destroy(par: *const xt_tgdtor_param) {
    // pr_info!("RConeNat: destroying rule");
}
