...
#[derive(Debug)]
struct PRConeNatTarget {
    // mappings: KVec<NatMapping>,  // NAT映射表
    target:u64,            // target结构体
}

impl kernel::Module for PRConeNatTarget {
    fn init(module: &'static ThisModule) -> Result<Self> {
        // pr_info!("PRConeNat: module being initialized\n");
        
        let mut target = KBox::new(xt_target::default(),flags::GFP_KERNEL)?;
        
        // 设置target属性
        let name = b"PRCONENAT";
        ...
        // 设置回调函数
        target.target = Some(prconenat_tg);
        target.checkentry = Some(prconenat_tg_check); 
        target.destroy = Some(prconenat_tg_destroy);
        ...
        pr_info!("PRConeNat: module initialized success\n");
        
        let full = PRConeNatTarget {
            // mappings: KVec::new(),
...
impl Drop for PRConeNatTarget {
    fn drop(&mut self) {
        pr_info!("PRConeNat: module being removed\n");
        ...
        unsafe { 
            ...
            // let module_ptr: *mut PRConeNatTarget = (*(self.target as *mut xt_target)).me as *mut PRConeNatTarget;
            // pr_info!("module {:X}\n",module_ptr as u64);
...
// target处理函数
extern "C" fn prconenat_tg(skb: *mut sk_buff, par: *const xt_action_param) -> u32 {
...
// 规则检查函数
extern "C" fn prconenat_tg_check(par: *const xt_tgchk_param) -> i32 {
    // pr_info!("PRConeNat: checking new rule");
    unsafe {
        let target = (*par).target; // 获取 xt_target 指针

        // if target.is_null() {
        //     pr_err!("PRConeNat: target is null!\n");
        //     return -1;
        // }
        // pr_info!("check {:X}\n",target as u64);
        // pr_info!("{:X}\n",(*target).me as u64);
        // 使用 container_of 获取 `PRConeNatTarget` 实例

        // let module_ptr: *mut PRConeNatTarget = (*target).me as *mut PRConeNatTarget;
        ...
        //     interface: 0,
        // },flags::GFP_KERNEL);
        // pr_info!("PRConeNat: Successfully accessed PRConeNatTarget {:X} num {}\n",module.target,module.mappings.len());
...
// 销毁函数
extern "C" fn prconenat_tg_destroy(par: *const xt_tgdtor_param) {
    // pr_info!("PRConeNat: destroying rule");
}
